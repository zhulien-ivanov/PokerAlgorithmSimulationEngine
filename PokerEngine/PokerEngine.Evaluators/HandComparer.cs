...
namespace PokerEngine.Evaluators
{
    public class HandComparer : IHandComparer
    {
        ...
        {
            var sortedHands = hands.OrderByDescending(x => (int)x.HandValue).ToList();

            var possibleWinners = new List<Hand>();
            var maxCombination = sortedHands[0].HandValue;

            possibleWinners.Add(sortedHands[0]);

            for (int i = 1; i < sortedHands.Count; i++)
            {
                if (sortedHands[i].HandValue != maxCombination)
                {
                    break;
                }

                possibleWinners.Add(sortedHands[i]);
            }

            var winningHands = new List<Hand>();

            winningHands.Add(possibleWinners[0]);

            for (int i = 1; i < possibleWinners.Count; i++)
            {
                var handResult = this.CompareHands(winningHands[0], possibleWinners[i], maxCombination);

                if (handResult == HandResult.HandsEqual)
                {
                    winningHands.Add(possibleWinners[i]);
                }
